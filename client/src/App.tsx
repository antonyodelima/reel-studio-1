...
function Sidebar({ mobileOpen, onClose }: { mobileOpen: boolean; onClose: () => void }) {
  ...
  return (
    <>
      {mobileOpen && <button aria-label="Close menu" onClick={onClose} className="fixed inset-0 z-30 bg-ink/30 backdrop-blur-sm lg:hidden" />}
      <aside className={cn("fixed inset-y-0 left-0 z-40 flex w-[252px] flex-col border-r border-ink/8 bg-paper px-4 py-5 transition-transform duration-300 lg:static lg:translate-x-0", mobileOpen ? "translate-x-0" : "-translate-x-full")}>
        ...
        <div className="mt-8">
          <p className="px-3 text-[10px] font-bold uppercase tracking-[.18em] text-muted">Workspace</p>
          <nav aria-label="Navigation" className="mt-2 space-y-1">
            {navItems.map((item) => {
              const Icon = iconMap[item.icon as keyof typeof iconMap];
              const active = item.href === "/" ? location === "/" : location.startsWith(item.href);
              return <Link key={item.href} href={item.href} onClick={onClose} className={cn("flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold transition", active ? "bg-white text-ink shadow-soft" : "text-muted hover:bg-ink/5 hover:text-ink")}><Icon className={cn("size-[17px]", active ? "text-coral" : "text-muted")} />{item.label}{item.label === "Projects" && <span className="ml-auto rounded-full bg-ink/6 px-2 py-0.5 text-[10px] text-muted">3</span>}</Link>;
            })}
          </nav>
        </div>
        <div className="mt-8">
          <p className="px-3 text-[10px] font-bold uppercase tracking-[.18em] text-muted">Resources</p>
          <div className="mt-2 space-y-1">
            <button onClick={() => toast.info("Creator guide is coming soon")} className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-sm font-semibold text-muted transition hover:bg-ink/5 hover:text-ink"><BookOpen className="size-[17px]" />Creator guide</button>
            <button onClick={() => toast.info(navigationHint)} className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-sm font-semibold text-muted transition hover:bg-ink/5 hover:text-ink"><Command className="size-[17px]" />Keyboard shortcuts<span className="ml-auto text-[10px] text-muted">⌘K</span></button>
          </div>
        </div>
        <div className="mt-auto rounded-2xl border border-ink/8 bg-white p-3 shadow-soft">
          <div className="flex items-center gap-3"><span className="grid size-9 place-items-center rounded-xl bg-coral/12 text-xs font-black text-coral">{initials}</span><div className="min-w-0"><p className="truncate text-xs font-bold">{ownerName}</p><p className="truncate text-[11px] text-muted">Local workspace</p></div><ChevronDown className="ml-auto size-4 text-muted" /></div>
          <div className="mt-3 flex items-center justify-between border-t border-ink/8 pt-3 text-[10px] font-semibold text-muted"><span>{navFooter}</span><button onClick={() => toast.info("You are already working locally")}><LogOut className="size-3.5" /></button></div>
        </div>
      </aside>
    </>
  );
